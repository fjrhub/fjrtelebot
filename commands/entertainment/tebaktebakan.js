...
module.exports = {
  ...
  async execute(ctx, args) {
    ...
    if (subcommand === "surrender") {
      const currentGame = getGame(chatId, "tebaktebakan");
      if (!currentGame) {
      ...
      clearGame(chatId, "tebaktebakan");
      return;
    }

    // 🚫 Prevent multiple games
    if (getGame(chatId, "tebaktebakan")) {
      return ctx.reply(
    ...
    try {
      const res = await axios.get(createUrl("siputzx", "/api/games/tebaktebakan"), {
        timeout: 8000,
      });
      ...
      if (result?.status && result.data) {
        ...
        setGame(chatId, "tebaktebakan", { answer });
